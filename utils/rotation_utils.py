...
def rotation_matrices_to_axes_angles(rotation_matrices):
    """
    Converts a stack of rotation matrices to rotation axes and angles.
    
    Args:
        rotation_matrices (torch.Tensor): A tensor of shape (K, 3, 3) representing K rotation matrices.
        
    Returns:
        axes (torch.Tensor): A tensor of shape (K, 3) representing the rotation axes.
        angles (torch.Tensor): A tensor of shape (K,) representing the rotation angles.
    """
    ...
    trace = torch.einsum('kii->k', rotation_matrices)
    ...
    # Handle the case where sin(theta) is very small to avoid division by zero
    sin_angles[sin_angles.abs() < 1e-6] = 1e-6
    
    axes = torch.stack([
        (R[:, 2, 1] - R[:, 1, 2]) / (2 * sin_angles[:, 0, 0]),
        (R[:, 0, 2] - R[:, 2, 0]) / (2 * sin_angles[:, 0, 0]),
        (R[:, 1, 0] - R[:, 0, 1]) / (2 * sin_angles[:, 0, 0])
    ], dim=1)
